...
public class Utils {
	...
	static void obtenerAccessToken(String foldername, String filename, Properties props, Scanner scanner) {
		try {
			...
			String userInputConsumerKey = "";
			userInputConsumerKey = props.getProperty("oauth.consumerKey" , null);
			if(userInputConsumerKey.trim().length() <= 0) {
				System.out.println("No se encontro el consumerKey, favor de proporcionarlo: ");
				userInputConsumerKey = scanner.next();
				scanner.nextLine();
				props.setProperty("oauth.consumerKey", userInputConsumerKey); 
			}
			String userInputConsumerSecret = "";
			userInputConsumerSecret = props.getProperty("oauth.consumerSecret" , null);
			if(userInputConsumerSecret.trim().length() <= 0) {
				System.out.println("No se encontro el consumerSecret, favor de proporcionarlo: ");
				userInputConsumerSecret = scanner.next();
				scanner.nextLine();
				props.setProperty("oauth.consumerSecret", userInputConsumerSecret); 
			}
	...
	public static Properties loadPropertiesFromFile(String foldername, String filename) throws IOException{
		...
		if (Files.exists(configFile)){
			...
			/* definir una interfaz predeterminada (java.util.function) y 
			 * almacenar una expresi�n lambda para validar si v comoString est� vac�o. 
			 * Si est� vac�o utilizar logger para informar al usuario.
			 * */
			// Escribe tu código aquí {
			BiConsumer<String, String> TW  = (T, U) -> {
				if(T.isEmpty() || U.isEmpty()) {
					logger.error("ERROR, no esta nada guardado tio");
				}
			};
			// }
